...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 z-50 max-w-md mx-auto">
        ...
        {TABS.map(({ id, label, Icon }) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] font-medium leading-tight ${active ? 'text-sky-500' : 'text-slate-400 dark:text-slate-500'}`}>
                {label}
              </span>
            </button>
          );
